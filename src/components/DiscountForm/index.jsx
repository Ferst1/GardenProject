...
//   return (
//     <div className={`${s.form_section} ${isAlternativeStyle ? s.alternative_form_section : ''}`}>
      ...
      <form onSubmit={handleSubmit(onSubmit)} className={`${isAlternativeStyle ? s.form_alternative : ''}`}>
          {Object.keys(input).map((key) => (
            <Input
              ...
              placeholder={input[key]}
              className={`${s.input} ${isAlternativeStyle ? s.input_alternative : ''}`}
              onChange={key === 'phone' ? (e) => {
                const inputValue = e.target.value;
                if (inputValue.length > 15) {
                  e.target.value = inputValue.slice(0, 15);
          ...
          <Button text={buttonText} ref={ButtonRef} onClick={displayNextError} className={`submit_button ${isSubmitted ? s.greenBackground : ''} ${isAlternativeStyle ? s.button_alternative : ''}`} />
        </form>
        {!isAlternativeStyle && <img src={HandsImage} alt="Hands holding garden tools" className={s.form_image} />}
      </div>
    </div>
